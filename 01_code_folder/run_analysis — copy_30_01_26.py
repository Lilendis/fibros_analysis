...
def main():
    ...
    ANALYSIS_CONFIG = {
        'min_vesicle_size': 1,          # ⭐ Минимальный размер везикулы в пикселях
        'max_vesicle_size': 200,        # ⭐ Максимальный размер везикулы в пикселях
        'collagen_percentile': 80,      # ⭐ Перцентиль коллагена для вычитания
        'vesicle_subtraction_factor': 0.9,      # ⭐ Фактор вычитания фона везикул (0-1)
        'protein_subtraction_percent': 90,      # ⭐ Процент вычитания IgG фона белка
        'vesicles_contrast_factor': 1.0,        # ⭐ Контраст везикул (1.0 = без изменений)
        'protein_brightness_factor': 1.7,       # ⭐ Яркость белка (>1 = ярче)
        'protein_contrast_factor': 1.5,         # ⭐ Контраст белка (>1 = контрастнее)
        'exclude_patterns': ['igg', 'pbs', 'control', 'neg'],  # ⭐ Паттерны для исключения
        'split_large_clusters': True,           # ⭐ Разделять большие скопления
        'intensity_ratio_threshold': 1.5,       # ⭐ Порог отношения интенсивностей
        'intensity_diff_threshold': 20,         # ⭐ Порог разности интенсивностей
        'background_method': 'original',    # ⭐ Метод вычитания фона
        'igg_database_path': IGG_DATABASE_PATH  # ⭐ Путь к базе данных IgG
    }
