...
"""
    ...
    def create_composite_image(self, sample_data, output_dir):
        ...
        try:
            ...
            # ⭐⭐⭐ ДОБАВИТЬ СЮДА ⭐⭐⭐
            print(f"      DEBUG: vesicles_binary sum = {np.sum(vesicles_binary) if vesicles_binary is not None else 'None'}")
            print(f"      DEBUG: vesicle_stats count = {len(vesicle_stats) if vesicle_stats else 0}")
            ...
            # Сегментация макрофагов ОТКЛЮЧЕНА (удалено по требованию)
            macrophages_labels = None
    ...
    def process_single_sample(self, sample_data, output_dir, gallery_root_dir, sample_num, total_samples):
        """Обработка одного образца (обновленная версия, без макрофагов)"""
        sample_name = sample_data['sample_name']
        ...
        # ⭐ Создаем детальный CSV для каждой везикулы
        vesicle_rows = []
        ...
        # ⭐ Собираем информацию для HTML галереи
        ...
        print(f"   ✅ РЕЗУЛЬТАТЫ:")
        print(f"      • Везикул всего: {summary['total_vesicles']}")
        print(f"      • Везикул в клетках: {summary['vesicles_in_cells']}")
        print(f"      • Колокализовано с белком: {summary['colocalized_vesicles']}")
        print(f"      • Процент в клетках: {summary['percent_in_cells']:.2f}%")
        print(f"      • Процент колокализации: {summary['colocalization_percentage']:.2f}%")
    ...
    def save_annotated_images(self, composite, vesicles_mask, macrophages_labels, vesicles_channel, safe_name, output_dir):
        """Сохраняет аннотированные изображения (макрофаги игнорируются)"""
        try:
            if vesicles_mask is not None and np.sum(vesicles_mask) > 0:
                # Композит с аннотациями (только везикулы, без макрофагов)
                composite_annotated = self.draw_vesicles_on_composite(
                    composite, vesicles_mask, vesicles_color=(0, 255, 255), thickness=2
                )
    ...
    def draw_vesicles_on_composite(self, composite, vesicles_binary, vesicles_color=(0, 255, 255), thickness=2):
        """Отрисовывает только везикулы (без макрофагов)"""
        if composite is None or vesicles_binary is None:
            return composite
        ...
        try:
            ...
            print(f"   Error drawing vesicles: {e}")
            return composite
    ...
    def subtract_collagen_preserve_vesicles(self, vesicles_channel_raw, cell_marker_channel, vesicles_binary):
        """Вычитает клеточный маркер только из фона, сохраняя везикулы"""
