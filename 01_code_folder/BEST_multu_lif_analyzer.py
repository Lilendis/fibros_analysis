...
class MultiSampleLifAnalyzer:
    def __init__(self, min_vesicle_size=5, max_vesicle_size=50, background_method='conservative', 
                subtraction_factor=0.7, vesicles_contrast_factor=3.0, protein_contrast_factor=1.5):
    ...
    def enhance_contrast(self, channel_data, contrast_factor=1.5):
    ...
    def enhance_vesicles_brightness(self, vesicles_channel, enhancement_factor=2.5):
        ...
        try:
            ...
            if current_max < 100:
                print(f"      ⚠️  Изображение темное, применяем агрессивное усиление")
    ...
    def save_igg_sample_images(self, sample_channels, sample_name, output_dir):
        ...
        try:
            ...
            if nuclei_channel is not None:
                nuclei_colored = self.apply_colored_channel(nuclei_channel, 'blue', keep_background_black=True)
            ...
            if collagen_channel is not None:
                collagen_colored = self.apply_colored_channel(collagen_channel, 'green', keep_background_black=True)
            ...
            if vesicles_channel is not None:
                vesicles_colored = self.apply_colored_channel(vesicles_channel, 'red', keep_background_black=True)
            ...
            if protein_channel is not None:
                protein_colored = self.apply_colored_channel(protein_channel, 'green', keep_background_black=True)
    ...
    def apply_colored_channel(self, gray_channel, color, keep_background_black=False):
        """Применяет цвет к серому каналу с опцией черного фона"""
        ...
        if keep_background_black:
            # ⭐ СОХРАНЯЕМ ЧЕРНЫЙ ФОН - только ненулевые пиксели получают цвет
            if color == 'red':
    ...
    def subtract_vesicles_background_interactive(self, vesicles_channel, collagen_channel, sample_name):
        ...
        try:
            ...
            while True:
                try:
                    user_input = input(f"      💬 Введите процент вычитания для везикул [{recommended_percent:.1f}%] (или 'skip' для пропуска): ").strip()
    ...
    def segment_vesicles(self, vesicles_channel):
        ...
        try:
            ...
            blurred = cv2.GaussianBlur(vesicles_channel, (3, 3), 0)
            
            # Адаптивный порог с более чувствительными настройками
            binary = cv2.adaptiveThreshold(
                blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                cv2.THRESH_BINARY, 15, 4  # Более чувствительные параметры
            )
    ...
    def enhance_vesicles_brightness(self, vesicles_channel, target_max_percentile=95):
        """Улучшает яркость везикул путем уменьшения максимальной яркости (как в Fiji)"""
        if vesicles_channel is None:
            return vesicles_channel
        ...
        try:
            ...
            print(f"      📊 Везикулы до улучшения: max={current_max:.1f}, 95%={current_95th:.1f}, mean={current_mean:.1f}")
            
            # Если изображение слишком темное, пропускаем улучшение
            if current_max < 50:
                print(f"      ⚠️  Изображение слишком темное, улучшение не применено")
                return vesicles_channel
            
            # Устанавливаем новый максимум на уровне 95% перцентиля
            new_max = current_95th
            
            # Масштабируем интенсивности
            vesicles_enhanced = vesicles_channel.astype(np.float32)
            vesicles_enhanced = (vesicles_enhanced / current_max) * new_max
            vesicles_enhanced = np.clip(vesicles_enhanced, 0, 255).astype(np.uint8)
            
            # Легкое увеличение контраста
            vesicles_enhanced = cv2.convertScaleAbs(vesicles_enhanced, alpha=1.2, beta=0)
            ...
            print(f"      ✅ Везикулы после улучшения: max={after_max:.1f}, 95%={after_95th:.1f}, mean={after_mean:.1f}")
            print(f"      🔧 Установлен новый максимум: {new_max:.1f} (было {current_max:.1f})")
